...
export default function KassaScreen({ navigation, route }) {
  ...
  const catRailHighlight = useTourHighlight('kassa.catRail');
  const productGridHighlight = useTourHighlight('kassa.productGrid');
  const cartHighlight = useTourHighlight('kassa.cart');
                ...
                >
                  ...
                  >
                    ...
                    {(item.modifiers||[]).length > 0 && (
                      <View style={styles.v2Mods}>
                        {(item.modifiers||[]).map((m,mi) => (
                          <Text key={mi} style={styles.v2Mod}>
...
const styles = StyleSheet.create({
  ...
  prePayClientRow: { flexDirection: 'row', alignItems: 'center', padding: 12, backgroundColor: 'rgba(240,160,80,0.08)', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(240,160,80,0.3)' },
  ...
  payMethodsRow:   { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  payMethodChip:   { flexGrow: 1, flexBasis: '45%', minHeight: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 14, paddingHorizontal: 12, borderRadius: 14, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface },
  payMethodChipActive: { borderColor: 'rgba(240,160,80,0.5)', backgroundColor: 'rgba(240,160,80,0.08)' },
  payMethodChipIcon:   { fontSize: 17, marginRight: 6 },
  payMethodChipTxt:    { fontFamily: fonts.familySemibold, fontSize: 15, color: colors.muted },

  payConfirmBtn: { marginTop: 16, paddingVertical: 16, borderRadius: 16, backgroundColor: 'rgba(240,160,80,0.85)', alignItems: 'center' },
  payConfirmText: { fontFamily: fonts.family, fontSize: 16, fontWeight: '700', color: '#fff' },

  // Кнопки модалки заметки
  noteModalBtnPrimary: { paddingVertical: 14, borderRadius: 14, backgroundColor: 'rgba(240,160,80,0.85)', alignItems: 'center' },
