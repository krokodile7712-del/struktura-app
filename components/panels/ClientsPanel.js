...
import { View, Text, StyleSheet, ScrollView, TextInput, Pressable, Animated, Alert } from 'react-native';
import EmptyState from '../EmptyState';
import { useFocusEffect } from '@react-navigation/native';
import { getAllClients, searchClients, getClientOrders, getTerms, pluralizeRu,
         getLoyaltyConfig, updateClientNote, updateClient, deleteClient } from '../../db/queries';
...
function ClientCard({ client, onNewOrder, onSaved, onDeleted, loyaltyModel, loyaltyConfig }) {
  ...
  const handleDelete = () => {
    Alert.alert(
      'Удалить клиента?',
      `«${client.fio}» будет удалён без возможности восстановить. История его прошлых заказов останется в Продажах, но без привязки к карточке.`,
      [
        { text: 'Отмена', style: 'cancel' },
        {
          text: 'Удалить', style: 'destructive',
          onPress: () => {
            try { deleteClient(client.id); onDeleted?.(); } catch (e) { console.error(e); }
          },
        },
      ]
    );
  ...
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 24, paddingBottom: 32 }} showsVerticalScrollIndicator={false}>
      ...
      {isAdmin && (
        <Pressable style={({ pressed }) => [styles.btnSec, { marginBottom: 8 }, pressed && { opacity: 0.88 }]}
          onPress={() => setEditing(e => !e)}>
          <Text style={styles.btnSecTxt}>{editing ? 'Скрыть' : '✎ Редактировать'}</Text>
        </Pressable>
      )}
      {isAdmin && (
        <Pressable style={({ pressed }) => [styles.btnDanger, pressed && { opacity: 0.88 }]}
          onPress={handleDelete}>
          <Text style={styles.btnDangerTxt}>Удалить клиента</Text>
        </Pressable>
      )}
...
export default function ClientsPanel({ navigation }) {
              ...
              {filtered.map((c, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.clientName, isActive && { color: colors.orange }]}>{c.fio}</Text>
                      <Text style={styles.clientSub}>
                        {loyaltyModel === 'points' ? `★ ${c.balance||0}` : `${c.visits||0} визит.`}
                      </Text>
                    </View>
...
const styles = StyleSheet.create({
  ...
  searchInput:{ flex: 1, backgroundColor: colors.surface2, borderRadius: 10, paddingVertical: 11, paddingHorizontal: 12, fontFamily: fonts.familyRegular, fontSize: 16, color: colors.text },
  addBtn:     { width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(240,160,80,0.15)', borderWidth: 1, borderColor: 'rgba(240,160,80,0.4)', alignItems: 'center', justifyContent: 'center' },
  ...
  clientRowDiv:  { borderBottomWidth: 1, borderBottomColor: colors.border },
  ...
  sectionAction:{ fontFamily: fonts.familySemibold, fontSize: 12, color: colors.orange },
  ...
  notePlaceholder: { fontFamily: fonts.familyRegular, fontSize: 13, color: colors.muted },
  ...
  btnDanger:  { paddingVertical: 13, borderRadius: 14, borderWidth: 1, borderColor: 'rgba(160,16,32,0.35)', backgroundColor: 'rgba(160,16,32,0.06)', alignItems: 'center' },
  btnDangerTxt: { fontFamily: fonts.familySemibold, fontSize: 13, color: colors.red },
  ...
  orderDiv:      { borderBottomWidth: 1, borderBottomColor: colors.border },
  ...
  orderChevron:  { fontSize: 18, color: colors.muted, transform: [{ rotate: '90deg' }] },
