...
export default function Sheet({ visible, onClose, onBack, title, children, sideWidth = 480, fixedHeightPct = null }) {
  ...
  // fixedHeightPct (например, список с flex:1 внутри) — минуем измерение
  // совсем, оно бы посчитало flex:1-содержимое пустым (нет доступного
  // места на момент замера) и заперло карточку на нулевой высоте списка.
  const bottomSheetHeight = fixedHeightPct != null
    ? screenHeight * fixedHeightPct
    : contentHeight != null
      ...
      <View style={[styles.overlay, isBottom ? { justifyContent: 'flex-end' } : { alignItems: 'flex-end' }]}>
        ...
        <Animated.View
          style={[
            ...
            isBottom
              ? {
                  ...(fixedHeightPct != null || contentHeight != null ? { height: bottomSheetHeight } : {}),
        ...
        >
          <View
            style={(!isBottom || fixedHeightPct != null || contentHeight != null) ? { flex: 1 } : undefined}
            onLayout={isBottom && fixedHeightPct == null && contentHeight == null ? (e => setContentHeight(e.nativeEvent.layout.height)) : undefined}
          >
            ...
            <View style={(isBottom && (fixedHeightPct != null || contentHeight != null)) ? { flex: 1 } : (isBottom ? { opacity: 0 } : { flex: 1 })}>
              {children}
            </View>
          </View>
        </Animated.View>
